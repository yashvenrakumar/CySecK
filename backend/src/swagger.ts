...
export const swaggerSpec = {
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "REST API for employee performance reviews and peer feedback. " +
  ...
  paths: {
    ...
    "/api/feedbacks": {
      get: {
        ...
        description:
          "Admin: all rows. Employee: pass `reviewerId` query (must match the reviewer). Each item includes who gave feedback and who it is about (review subject).",
        security: [{ RoleHeader: [] }],
        parameters: [
          {
            name: "x-role",
    ...
    "/api/feedbacks/{id}": {
      patch: {
        ...
        description:
          "Change comment or rating on feedback you authored. `reviewerId` in the body must match the stored reviewer. Only allowed while the linked review is **open**.",
        security: [{ RoleHeader: [] }],
        parameters: [
          {
            name: "id",
